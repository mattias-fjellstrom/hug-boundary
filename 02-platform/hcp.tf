...
resource "hcp_hvn" "this" {
  hvn_id         = "hvn-${var.hcp_vault_cloud_provider}-${var.aws_region}"
  cidr_block     = var.hcp_virtual_network_cidr
  cloud_provider = var.hcp_vault_cloud_provider
...
resource "hcp_vault_cluster" "this" {
  cluster_id      = "vault-${var.hcp_vault_cloud_provider}-${var.aws_region}"
  hvn_id          = hcp_hvn.this.hvn_id
